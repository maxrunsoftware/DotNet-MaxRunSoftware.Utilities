...
/// </summary>
public class MicrosoftSqlServerProperties
{
    static MicrosoftSqlServerProperties()
    {
        var propertiesAll = typeof(MicrosoftSqlServerProperties)
            .GetProperties(BindingFlags.Public | BindingFlags.Instance)
            .OrderByOrdinalIgnoreCaseThenOrdinal(o => o.Name)
            .ToImmutableArray();

        properties = propertiesAll.Where(o => o.CanWrite).ToImmutableArray();
        var propertiesNames = properties.Select(o => o.Name).ToHashSet();

        propertiesCustom = propertiesAll.Where(o => !propertiesNames.Contains(o.Name)).ToImmutableArray();
    }
    private static readonly ImmutableArray<PropertyInfo> properties;
    private static readonly ImmutableArray<PropertyInfo> propertiesCustom;
    ...
    public MicrosoftSqlServerProperties(Sql sql)
    {
        ...
        for (var i = 0; i < properties.Length; i++)
        {
            ...
            var obj = result[i];
            var objConverted = ConvertValue(property, obj);
            ...
            property.SetValue(this, objConverted);
        }
    }

    private List<(string Name, string Value)> ToStringProperties() => properties.Concat(propertiesCustom)
    ...
    /// <br/>For a clustered instance of SQL Server on a failover cluster, this value changes as the instance of SQL Server fails over to other nodes in the failover cluster.
    /// <br/>On a stand-alone instance of SQL Server, this value remains constant and returns the same value as the MachineName property.
    /// <br/>Note: If the instance of SQL Server is in a failover cluster and you want to obtain the name of the failover clustered instance, use the MachineName property.
    /// <br/>NULL = Input isn't valid, an error, or not applicable.
    /// <br/>Base data type: nvarchar(128)
    /// </summary>
    ...
    /// <br/>NULL = Input isn't valid, an error, or not applicable.
    /// <br/>Base data type: int
    /// <br/>For availability replicas to be created and run on an instance of SQL Server, Always On availability groups must be enabled on the server instance. For more information, see Enable and Disable Always On Availability Groups (SQL Server).
    /// <br/>Note: The IsHadrEnabled property pertains only to Always On availability groups. Other high availability or disaster recovery features, such as database mirroring or log shipping, are unaffected by this server property.
    /// </summary>
    ...
    /// <summary>
    /// Applies to: SQL Server 2019 (15.x) and later.
    /// <br/>Returns 1 if tempdb has been enabled to use memory-optimized tables for metadata; 0 if tempdb is using regular, disk-based tables for metadata. For more information, see tempdb Database.
    /// <br/>Base data type: int
    /// </summary>
    ...
    /// <br/>For a clustered instance, an instance of SQL Server running on a virtual server on Microsoft Cluster Service, it returns the name of the virtual server.
    /// <br/>NULL = Input isn't valid, an error, or not applicable.
    /// <br/>Base data type: nvarchar(128)
    /// </summary>
