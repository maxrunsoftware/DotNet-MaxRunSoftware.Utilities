...
public class SqlOracle : Sql
{
    ...
    private bool ShouldStop(string database, out string dbName)
    {
        ...
        if (database != null)
        {
            if (dbName == null)
            {
                log.LogDebug("Requested database name '{Database}' does not match our connected database because could not determine current database name, proceeding anyway", database);
                return false;
            }

            if (!dbName.EqualsIgnoreCase(database))
            {
                log.LogDebug("Requested database name '{Database}' does not match our connected database '{DbName}' so we are not continuing request", database, dbName);
                return true;
            }
        }
        ...
        if (dbName != null)
        {
            ...
            {
                log.LogDebug($"Requested database name '{database}' is in our list of {nameof(ExcludedDatabases)} so we are not continuing request");
                return true;
            }
        }
