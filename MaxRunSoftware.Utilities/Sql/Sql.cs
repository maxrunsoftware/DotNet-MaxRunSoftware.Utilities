...
public abstract class Sql
{
    ...
    public virtual void Insert(string database, string schema, string table, Table tableData, ushort? batchSize = null)
    {
        ...
        if (bs > bsm)
        {
            log.LogWarning($"Requested {nameof(InsertBatchSize)} of {bs} is greater then {nameof(InsertBatchSizeMax)} of {bsm} so using {bsm}");
            bs = bsm;
        }
            ...
            foreach (var row in tableData)
            {
                ...
                if (currentParameterCount + countColumns > bs || currentRow == countRows)
                {
                    ...
                    if (sqlText != null)
                    {
                        ...
                        log.LogTrace("ExecuteNonQuery: {CommandText}", command.CommandText);
    ...
    private Dictionary<string, SqlObjectTableColumn> InsertGetTableColumns(string database, string schema, string table)
    {
        ...
        catch (Exception e) { log.LogDebug(e, "Unable to obtain column list for database:{Database}  schema:{Schema}  table:{Table}", database, schema, table); }
    ...
    public void ExecuteQuery(string sql, Action<IDataReader> action, params SqlParameter[] parameters)
    {
        ...
        using (var command = CreateCommand(connection, sql))
        {
            ...
            log.LogTrace("ExecuteQuery: {Sql}", sql);
    ...
    public int ExecuteNonQuery(string sql, params SqlParameter[] parameters)
    {
        ...
        {
            AddParameters(command, parameters);
            log.LogTrace("ExecuteNonQuery: {Sql}", sql);
    ...
    public object ExecuteScalar(string sql, params SqlParameter[] parameters)
    {
        ...
        {
            AddParameters(command, parameters);
            log.LogTrace("ExecuteScalar: {Sql}", sql);
    ...
    public Table[] ExecuteStoredProcedure(string schemaAndStoredProcedureEscaped, params SqlParameter[] parameters)
    {
        ...
        {
            AddParameters(command, parameters);
            log.LogTrace("ExecuteStoredProcedure: {SchemaAndStoredProcedureEscaped}", schemaAndStoredProcedureEscaped);
