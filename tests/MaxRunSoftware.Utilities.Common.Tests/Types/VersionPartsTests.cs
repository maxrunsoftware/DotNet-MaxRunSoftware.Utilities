...
public class VersionPartsTests(ITestOutputHelper testOutputHelper) : TestBaseBase(testOutputHelper)
{
    ...
    public void ValueInt(string expected, string version) => Assert.Equal(
        ...
        new VersionParts(version).Select(o => o.ValueInt).ToArray()
    );
    ...
    public void ValueLong(string expected, string version) => Assert.Equal(
        ...
        new VersionParts(version).Select(o => o.ValueLong).ToArray()
    );
    ...
    public void ValueBigInteger(string expected, string version) => Assert.Equal(
        ...
        new VersionParts(version).Select(o => o.ValueBigInteger).ToArray()
    );

    [SkippableFact]
    public void Value()
    {
        var v = new VersionParts("1.2.3.4.5");
    ...
    {
        var list = new List<VersionParts>();
        var versions = new[] { a, b, c, d, e }.Select(o => new VersionParts(o)).ToArray();
    ...
    public void IsPreRelease(string a) => Assert.True(new VersionParts(a).IsPreRelease);
    ...
    {
        var v1 = new VersionParts(a);
        var v2 = new VersionParts(b);
        Assert.True(VersionParts.Equals(v1, v2));
        Assert.True(VersionParts.Equals(v2, v1));
    }
    ...
    public void NotEqual(string a, string b)
    {
        var v1 = new VersionParts(a);
        var v2 = new VersionParts(b);
        Assert.False(VersionParts.Equals(v1, v2));
        Assert.False(VersionParts.Equals(v2, v1));
    }
}
